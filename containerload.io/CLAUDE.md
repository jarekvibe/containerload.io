...
Die geometrischen Lücken stehen im Dialog als **nummerierte Zonen-Karten** (Maß, „Bewegung möglich", Grund mit den angrenzenden Packstücken oder der Wand, Empfehlung); dieselben Nummern hängen als Plaketten (CanvasTexture-Sprites) über den 3D-Zonen — Karten und Plaketten zählen wörtlich dieselbe `sichZonenAlle`-Liste, sonst müsste man raten, welche Karte welche Lücke meint. Jede Zone trägt dafür `art`, `luecke`, `nachbarn` (angrenzende ti) und `wand`. Die Möbel-Vorschau: Luftkissen bis `SICH.POLSTER` (45 cm — echte Staupolster füllen auch mittelbreite Lücken; 30 war zu konservativ, Jarek kam mit Vergleichsfoto), darüber Stauholz-Verband mit Pfosten an beiden Lückenflächen (vier Balken in zwei Höhen, vier Pfosten — mehr wurde Rauschen), an der Tür Sperrstangen; Balken und Pfosten tragen dieselben dunklen Kantenlinien wie die Ladungskisten — erst die machen es zum Teil des Bildes.

**Die Hilfsmittel sind Vorschlag, nicht Ladung — und sehen auch so aus.** Erster Versuch waren Realfarben (weißes Kissen, Holzton, Stahlgrau); im stilisierten Bild wirkte das „billig" — Jareks Wort, und er hatte recht: fotorealistische Farben in einer Flat-Szene lesen sich als misslungener Realismus. Jetzt tragen alle drei Materialien EINEN Ton, das Zonen-Orange, transluzent (`0xdda24c`, Kissen 0.55, Holz/Stahl 0.45, `depthWrite: false`) — dieselbe Sprache wie der Platzier-Ghost im manuellen Modus: „das ist noch nicht real". Die Zonen-Füllung ging dafür auf 0.06 runter, damit Zone und Möbel sich nicht zu einem Orange-Klumpen addieren.

...
`/admin` (noindex, robots-Disallow, saubere Pfade in `_redirects`): eigenes HTML ohne React/Tailwind im Werkzeug-Look, Token in localStorage, Charts als Inline-SVG. Zwei Fallen von der ersten Fassung: ein CSS-`display: flex` auf `#login` überstimmt das `hidden`-Attribut (deshalb `[hidden]{display:none!important}`), und `width` auf `<span>`-Balken tut nichts ohne `display: block`. Chart-Farben validiert (dataviz-Regeln): `#2E8FFF` + `#BA8232` auf `#0E1116` — das helle Zonen-Orange `#DDA24C` fiel durch die Leuchtdichte-Prüfung.
